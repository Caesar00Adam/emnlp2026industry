...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.val_before_train:
            ...
            print(val_metrics,self.global_step,flush=True)
            if self.config.trainer.val_only:
                self.logger.log(data=val_metrics, step=self.global_step)
                print(val_metrics,self.global_step,flush=True)
                print("\n" + "="*40, flush=True)
                print("🎯 [成功获取验证指标] val_metrics:", val_metrics, flush=True)
                print("📌 [当前 Step]:", self.global_step, flush=True)
                print("="*40 + "\n", flush=True)
                return

        self.data_iterator = iter(self.train_dataloader)
        bert_train_interval = max(1, self.config.worker.reward.bert_train_interval)
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    ...
                    if self.config.worker.reward.use_bert_reward and self.global_step % bert_train_interval == 0:
                        with timer("train_bert_reward", timing_raw):
                            bert_trained = ray.get(self.reward_fn.train_bert_model.remote(batch))
                            metrics["reward/bert_trained"] = float(bert_trained)
